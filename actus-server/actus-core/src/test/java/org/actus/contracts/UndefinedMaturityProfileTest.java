...
public class UndefinedMaturityProfileTest {
    ...
    public Stream<DynamicTest> test() {
        String testFile = "./src/test/resources/actus/actus-tests-ump.json";
        ...
        return testIds.stream().map(testId -> {
            ...
            ArrayList<ContractEvent> schedule = UndefinedMaturityProfile.schedule(LocalDateTime.parse(test.getto()), terms);
            schedule = UndefinedMaturityProfile.apply(schedule, terms, observer);
